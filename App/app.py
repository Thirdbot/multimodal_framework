...
from modules.inference import InferenceManager, InferenceConfig
...
# --- Helper Functions ---
def run_command_with_output(command, output_widget, on_complete=None):
    """Runs a shell command and streams output to a text widget."""
...
class DashboardView(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.vars = Variable()
        datasets_count = len(list(self.vars.DATASET_FORMATTED_DIR.glob("*"))) if self.vars.DATASET_FORMATTED_DIR.exists() else 0
        models_count = len(list(self.vars.REGULAR_MODEL_DIR.glob("*"))) + len(list(self.vars.VISION_MODEL_DIR.glob("*")))
        
        ctk.CTkLabel(self.stats_frame, text=f"Local Datasets: {datasets_count}", font=ctk.CTkFont(size=14)).pack(side="left", padx=20)
        ctk.CTkLabel(self.stats_frame, text=f"Local Models: {models_count}", font=ctk.CTkFont(size=14)).pack(side="left", padx=20)
...
class ModelRepositoryView(ctk.CTkFrame):
    ...
    def refresh_list(self):
        ...
        for label, path in model_locations:
            ...
            ctk.CTkLabel(self.scroll_frame, text=label, font=ctk.CTkFont(weight="bold")).pack(fill="x", padx=5, pady=(10, 5))
...
class DownloadHubView(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.entry_search = ctk.CTkEntry(self.search_frame, placeholder_text="Search models or datasets (e.g., Qwen/Qwen1.5-0.5B)")
        self.entry_search.pack(side="left", fill="x", expand=True, padx=10, pady=10)
        
        self.btn_search = ctk.CTkButton(self.search_frame, text="Search / Download")
...
class InferenceView(ctk.CTkFrame):
    ...
    def run_inference(self, msg):
        ...
        if not model_dirs:
            self.after(0, lambda: self.append_response("System: No fine-tuned models found. Please train a model or specify a path in Configuration."))
            return
...
class ConfigView(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.btn_reload = ctk.CTkButton(self.file_frame, text="Reload", width=80, command=lambda: self.load_file(self.file_combo.get()))
...
class TrainingView(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.btn_start = ctk.CTkButton(self.controls_frame, text="Start Training", command=self.start_training, fg_color="green")
        self.btn_start.pack(side="left", padx=10, pady=10)
        
        self.btn_stop = ctk.CTkButton(self.controls_frame, text="Stop", command=self.stop_training, fg_color="red", state="disabled")
...
class TopToolbar(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.btn_file = ctk.CTkButton(self, text="File", width=60, fg_color="transparent", text_color=("gray10", "#DCE4EE"), hover_color=("gray70", "gray30"))
        self.btn_file.pack(side="left", padx=5)
        
        self.btn_view = ctk.CTkButton(self, text="View", width=60, fg_color="transparent", text_color=("gray10", "#DCE4EE"), hover_color=("gray70", "gray30"))
        self.btn_view.pack(side="left", padx=5)
        
        self.btn_help = ctk.CTkButton(self, text="Help", width=60, fg_color="transparent", text_color=("gray10", "#DCE4EE"), hover_color=("gray70", "gray30"))
